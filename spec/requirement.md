...
- 用户确认语义必须显式建模为确认、拒绝、追问三类；先用规则判断高置信确认/拒绝，规则不命中时调用 LLM 做结构化语义判断；低置信、解析失败或 provider 失败都必须 fail closed 为追问，不能误触发写操作。
- LLM 语义确认分类本身必须可观测：分类 provider 调用写入 `agent_llm_attempt`，结构化分类结果写入 `confirmation_intent_llm` event，避免 human-in-the-loop 策略判断成为隐藏外部调用。
- 工具参数校验失败必须区分 fatal 与 recoverable。缺少可由用户补充的参数时，tool precheck result 应包含 `recoverable=true`、`nextActionRequired=user_input` 和明确追问文本，run 进入 `PAUSED` 而不是终态失败。
...
- 可恢复缺槽进入 `PAUSED`，`final.nextActionRequired=user_input`，用户补充信息后通过 continuation 继续同一个 run
